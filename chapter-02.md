...
Основною одиницею мови HTML є теґ. За допомогою теґів описується вся структура HTML-документа. Теги оформляються кутовими дужками <ім'я теґу>, між якими прописується ім'я теґу.

Практично всі теґи мають **атрибути** (глобальні, що застосовуються для всіх html-елементів, і власні), які вказуються у форматі **параметр = "значення"**. Атрибути дозволяють змінювати властивості елемента, для якого вони задані. Атрибути прописуються в початковому тезі елемента.
...
Більшість теґів - парні, вони складаються з початкового і кінцевого теґів. Початковий теґ показує, де починається елемент, кінцевий - де закінчується. Закриваючий теґ утворюється шляхом додавання слешу / перед ім'ям теґа: `<ім'я_тега> ... </ім'я_тега>`. Між початковим і кінцевим теґами знаходиться вміст теґа - контент.

Одиночні теґи не можуть зберігати в собі вмісту безпосередньо, він прописується як значення атрибута, наприклад, теґ `<input type = "button" value = "Кнопка">`

### Будова HTML-документа

HTML слідує правилам, які містяться у файлі оголошення типу документа (Document Type Definition, або DTD). DTD являє собою XML-документ, що визначає, які теґи, атрибути та їх значення дійсні для конкретного типу HTML. Для кожної версії HTML є свій DTD.
...
Елементи, що знаходяться всередині теґа **DOM (document object model)** . При цьому елемент є кореневим елементом.
...
**Предок** - елемент, який містить в собі інші елементи. Gредком для всіх елементів є `<html>`. У той же час елемент `<body>` є предком для всіх містяться в ньому теґів: `<h1>`,`<p>`, `<span>`, `<nav>`і т.д.
...
### Підготовка необхідного програмного забезпечення 

Для виконання завдання неохідно встановити та налаштувати консольне застосування **Telnet** або його аналог. Telnet - це мережна утиліта, яка дозволяє з'єднатися з віддаленим портом будь-якого комп'ютера та встановити інтерактивний канал зв'язку, наприклад, передачі команд або отримання інформації.

Вбудований клієнт telnet для ОС Windows має суттєві проблеми з виведенням інформації у консоль тому бажано використати або клієнт telnet для ОС сімейства Unix або встановити клієнт для ОС Windows **Putty**. Саме на прикладі роботи з Putty побудовано подальше викладення матеріалу.

Для роботи необхідно завантажити і встановити на комп'ютер утиліту Putty.

Після запуску необхідно зробити наспуні налаштування для тестування на helpnik.college.ks.ua:

![Налаштуванн Putty для тесту](img/02-050.png)
![Налаштуванн Putty для тесту](img/02-051.png)

Окрім того, необхідно заздалегідь підготувати у текстовому редакторі заготовку заголовку запиту. Для тестування на helpnik.college.ks.ua
```
GET / HTTP/1.1
Host:helpnik.college.ks.ua
Connection:close
```
В теорії цей запит можна набирати безпосередньо в консолі, але швидкість формування запиту має бути досить висока, щоб з'єднання не було завершене сервером у зв'язку з перевищеням часу відправлення запиту.

Після цього необхідно розпочати під'єднання до сервера та вставити у консоль заготовлені рядки запиту.
![Відкриття з'єднання в Putty](img/02-052.png)
Після додавання рядків необхадно два рази натиснути **Enter** для інформування сервера про те, що передача тіла запиту завершена.

Якщо ви виконали завдання правильно, то побачите наступне повідомлення та код сторінки в консолі:
![Завершення з'єднання в Putty](img/02-053.png)

Для аналізу відповіді серверу відкрийте файл `c:\putty.log` в якому знайдіть наступну інформацію:
- дата та час відповіді;
- назва програми web-сервера;
- кодування тіла відповіді.

Спробуйте визначити призначення та суть змісту інших заголовків відповіді сервера.

### Створення HTTP-запиту та аналіз HTTP-відповіді 

Виконайте самостійно дії, що описані у попередньому підрозділі, для сайтів `do.college.ks.ua`, `google.com`, `ukr.net`.

Проаналізуйте зміст заголовків, опишіть коротко, яку інформацію можна отримати із заголовків відповіді про сервер.
